...
export default {
  inject: ['refList', 'formConfig', 'getGlobalDsv', 'globalOptionData', 'globalModel', 'getOptionData', 'getReadMode'],
  ...
  methods: {
    ...
    initFileList() {
      //初始化上传组件的已上传文件列表
      if ((this.field.type !== 'image' && this.field.type !== 'file') || this.designState === true) {
        return
      }
    ...
    * */
    setValue(newValue) {
      /* if ((this.field.type === 'image') || (this.field.type === 'file-upload')) {
    ...
    resetField() {
      ...
      console.log(this.field.type, 'field reset to default value:', defaultValue)
      //清空上传组件文件列表
    ...
    clearSelectedOptions() {
      ...
      if (
        this.field.type === 'checkbox' ||
        this.field.type === 'tag' ||
        ((this.field.type === 'select'||this.field.type === 'cascader') && this.field.options.multiple)
      ) {
        this.fieldModel = []
      } else {
        this.fieldModel = ''
