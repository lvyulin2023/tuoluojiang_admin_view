<template>
  ...
  >
    <div v-show="!getReadMode()" ref="fieldEditor">
      <div class="pic-list">
        <div v-if="fieldModel.length" class="pic" v-for="(img, index) in fieldModel" :key="index">
          <img class="img" :src="img.url" />
          <div class="pic-upload">
            <div class="pic-upload-content">
              <i @click="handlePictureCardPreview(index)" class="el-icon-zoom-in"></i>
        ...
        <i
          @click="beforeUpload"
          v-if="fieldModel.length < field.options.limit"
    ...
    <template>
      <div v-if="getReadMode()">
        <div v-if="fieldModel.length" class="pic-list">
          <div class="pic" v-for="(img, index) in fieldModel" :key="index">
            <img class="img" :src="img.url" />
            <div class="pic-upload">
              <div class="pic-upload-content">
                <i @click="handlePictureCardPreview(index)" class="el-icon-zoom-in"></i>
    ...
    >
      <upload-picture
        ref="uploadPicture"
        :maxNum="Number(field.options.limit)"
        :multiple="true"
        :check-button="true"
        @getImage="getImage"
      ></upload-picture>
...
export default {
  ...
  methods: {
    handlePictureCardPreview(i) {
      this.srcList.push(this.fieldModel[i].url)
      this.isImage = true
    },
    handleAvatar(index) {
    ...
    beforeUpload() {
      ...
      let imgFile = this.fieldModel.map((item) => {
        return {
          att_size: item.size,
          real_name: item.name,
          att_dir: item.url
      ...
      // setTimeout(() => {
      // this.$refs.uploadPicture.checkPicList = imgFile
      // }, 300)
    },
    getImage(data) {
      let imgFile = data.map((item) => {
        return {
          size: item.att_size,
          name: item.real_name,
          url: item.att_dir
      ...
      this.fieldModel = [...this.fieldModel, ...imgFile]
      this.handleClose()
