  ...
  >
    <template v-for="(widget, index) in widgetList">
      ...
      <template v-else>
        ...
        >
          <!-- 递归传递插槽！！！ -->
          <template v-for="(slot, index) in Object.keys($scopedSlots)" v-slot:[slot]="scope">
            <slot :name="slot" v-bind="scope" />
          </template>
        </component>
      </template>
...
export default {
  ...
  computed: {
    formConfig() {
      if (this.formJsonObj.formConfig) {
  ...
  methods: {
    ...
    resetForm() {
      ...
      let wNameList = Object.keys(this.widgetRefList)
      wNameList.forEach((wName) => {
        // console.log(wName, 'this.widgetRefList[wName]')
        let foundW = this.getWidgetRef(wName)
        if (foundW.fileList && foundW.fileList.length) {
          foundW.resetField()
        }
