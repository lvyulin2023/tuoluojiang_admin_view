  ...
  methods: {
    ...
    nextStep() {
      if (this.activeIndex == 1) {
        ...
        if (this.from.members.length == 0) return this.$message.error('请选择需要考勤的人员或者部门')
        if (this.from.admins.length == 0) return this.$message.error('请选择需要考勤组的管理人员')
    ...
    saveFn(data) {
      if (this.activeIndex == 1 && this.type == 'add') {
        saveAttendanceGroup(data).then((res) => {
          this.id = res.data.id
          this.activeIndex = this.activeIndex + 1
        })
      } else {
        putAttendanceGroup(this.id, data).then((res) => {
          this.activeIndex = this.activeIndex + 1
        })
      }
    },
