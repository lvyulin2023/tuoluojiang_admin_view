...
<template>
  <div class="divBox">
      ...
      <el-row>
        ...
        <el-col class="assess-right" v-bind="gridr">
          ...
          <el-table ref="table" :data="list" class="table" row-key="id" style="width: 100%">
            <el-table-column label="职位" min-width="200" prop="position">
          ...
          <div v-hasPermi="['hr:training:promotion:add']" class="add-row" @click="addANewLine">
            <span class="el-icon-plus"></span> 添加行
          </div>
        </el-col>
      </el-row>
    </el-card>
    ...
    <!-- 晋升标准侧滑 -->
    <el-drawer :before-close="handleClose" :visible.sync="editDrawer" direction="rtl" size="60%" title="晋升标准">
      <div class="check-box">
...
export default {
  ...
  methods: {
    ...
    // 编辑
    editFn(row) {
      let newArr = this.selectedList.filter((v) => row.position.every((val) => val != v))
    ...
    async addRow(row) {
      ...
      if (!row.id) {
        await savePromotionDataApi(data)
        this.selectedList = [...this.selectedList, ...data.position]
        row.show = false
      } else {
