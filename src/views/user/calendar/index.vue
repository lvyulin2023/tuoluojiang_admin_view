            ...
            <full-calendar ref="calendar" id="calendar" style="height: calc(100vh - 150px)" :options="calendarOptions">
              ...
              <template v-slot:eventContent="arg">
                <div
                  class="item"
                  :style="{
                    background: period == 3 ? getColor(arg.event.textColor, '0.1') : arg.event.color
                  }"
                >
...
export default {
  ...
  mounted() {
    ...
    this.formInfo.avatar = JSON.parse(localStorage.getItem('userInfo')).avatar
  ...
  methods: {
    ...
    putStatus(text, status, item) {
      ;(this.item = item.extendedProps), (this.status = status)
      if (item.extendedProps.cid_value == 3 || item.extendedProps.cid_value == 4) {
        let id =
          item.extendedProps.bill_id.bill_id !== 0
            ? item.extendedProps.bill_id.bill_id
            : item.extendedProps.bill_id.remind_id
        if (status == 3) {
          clientRemindDetailApi(id).then((res) => {
            res.data.id = item.extendedProps.bill_id.remind_id
          ...
          // this.scheduleRecord(item.extendedProps, status)
        }
      } else if (item.extendedProps.cid_value == 2) {
        if (item.extendedProps.finish == 3) {
          return false
        }

        this.formInfo.follow_id = item.extendedProps.bill_id ? item.extendedProps.bill_id.follow_id : 0
        this.formInfo.data.id = item.extendedProps.link_id
    ...
    async scheduleRecord(data, status) {
      ...
      let id = data ? data.itemId : this.item.itemId
    ...
    prev() {
      ...
      if (this.period == 2) {
        ...
        this.$refs.calendarBar.value = this.$moment(this.$moment(date).subtract(7, 'd')).format('YYYY-MM-DD')
      } else {
    ...
    getList() {
      this.loading = true
      if (this.period == 1) {
        this.start_time = this.$moment(this.calendarApi.view.currentStart).format('YYYY-MM-DD') + ' 00:00:00'
        this.end_time = this.$moment(this.calendarApi.view.currentStart).format('YYYY-MM-DD') + ' 23:59:59'
      } else if (this.period == 2) {
        this.start_time = this.$moment(this.calendarApi.view.currentStart).format('YYYY-MM-DD') + ' 00:00:00'
        this.end_time =
          this.$moment(this.calendarApi.view.currentEnd).subtract(1, 'days').format('YYYY-MM-DD') + ' 23:59:59'
      } else {
      ...
      scheduleListApi(data).then((res) => {
        ...
        res.data.map((item) => {
          ...
          if (this.$moment(item.end_time).format('YYYY-MM-DD') != this.$moment(item.start_time).format('YYYY-MM-DD')) {
            item.all_day = true
            item.end_time = this.$moment(item.end_time).format('YYYY-MM-DD') + ' 24:00:00'
          } else if (
            this.$moment(item.end_time).format('YYYY-MM-DD') == this.$moment(item.start_time).format('YYYY-MM-DD') &&
            item.all_day == 1
          ) {
            item.all_day = true
            item.end_time = this.$moment(item.end_time).format('YYYY-MM-DD') + ' 24:00:00'
          ...
          newArr.push({
            ...
            start_time,
            end_time,
            color: this.getColor(item.color, '0.1'),
