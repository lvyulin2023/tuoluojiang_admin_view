...
export default {
  ...
  data() {
    return {
      ...
      noon: this.$moment(new Date()).format('a'),
      pend: {
        status: 3
      },
      dialogVisible: false,
      ...
      value: new Date(),
      calendar: {
        time: this.$moment(new Date()).format('YYYY-MM') // 当月
      },
      dailyList: {}, // 点击当前天数的计划与总结
      currentDay: this.$moment(new Date()).format('DD'), // 当天
      dailyDay: [], // 当月有日报的天数组合
      imgList: [],
  ...
  computed: {
    hasFinished() {
      let arrList = []
      this.needList.map((item) => {
        arrList.push(item)
      })
      return arrList.some((item) => item.finish >= 2)
    },
    hasUnfinished() {
  ...
  methods: {
    ...
    setScheduleRecord(item, status) {
      ...
      if (item.cid == 3 || item.cid == 4) {
        ...
        this.formInfo.data.id = item.link_id
