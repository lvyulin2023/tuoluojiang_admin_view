    ...
    <el-table :data="debtData" style="width: 100%">
      ...
      <el-table-column prop="status" label="付款审核状态" min-width="100">
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.status === 1" type="info" size="mini"> 已通过</el-tag>
      ...
      <el-table-column prop="address" fixed="right" min-width="180" :label="$t('public.operation')">
        <template slot-scope="scope">
          ...
          <template v-if="scope.row.status !== 2">
            <el-button @click="handleDelete(scope.row)" v-if="scope.row.status === 2" type="text"
              >{{ $t('public.delete') }}
            </el-button>
          </template>
        </template>
...
import { clientBillDeleteApi, clientBillListApi, withdrawApi, getContractStatisticsApi } from '@/api/enterprise'
...
export default {
  ...
  methods: {
    ...
    // 撤回
    withdraw(val) {
      this.$modalSure(this.$t('您确定要撤回此账目记录吗')).then(() => {
