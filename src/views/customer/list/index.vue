        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              >
              <span
                v-else-if="(header.field === 'c45aa76a' && header.type == 'checked') || header.field === 'area_cascade'"
              >
              ...
              <div v-else-if="header.field === 'customer_status'">
                <el-tag v-if="scope.row[header.field] == 0" class="customer-tag" size="small">未成交</el-tag>
          ...
          <el-table-column fixed="right" label="操作" prop="address" show-overflow-tooltip width="220">
            ...
            <template slot-scope="scope">
              <el-button v-hasPermi="['customer:list:check']" type="text" @click="handleCheck(scope.row)"
                >查看</el-button
              >
              <el-button v-if="types !== 3" type="text" @click="handleFollowUp(scope.row)">填写跟进</el-button>
              ...
              <el-dropdown>
                ...
                <el-dropdown-menu style="text-align: center">
                  <el-dropdown-item v-if="types !== 3" @click.native="addContract(scope.row)">
                    添加合同
                  </el-dropdown-item>
...
export default {
  ...
  methods: {
    ...
    // 下载模板
    exportTemplate() {
      this.gettime = this.$moment(new Date()).format('YYYY-MM-dd HH:mm:ss')
      fileLinkDownLoad(this.fileLinkDownLoadUrl, '客户合同导入模板' + this.gettime + '.xlsx')
    },
    // 导出列表数据
    async exportList() {
      if (this.total > 1000) {
    ...
    handleFollow(row, type) {
      var status = null
      status = type === 1 ? (row.follow === 0 ? 1 : 0) : row.up_follow === 0 ? 1 : 0
      const data = {
        status,
        types: 1
      }
    ...
    handleLabelConf(res, id) {
      ...
      if (res.list.length <= 0) {
        this.$message.error(this.$t('customer.placeholder58'))
      } else {
        if (id) {
          data.push(id.id)
        } else if (!id) {
    ...
    importExcelData(arrRes) {
      ...
      customerImport(this.types, data).then((res) => {
        this.getTableData('')
      })
    }
  }
