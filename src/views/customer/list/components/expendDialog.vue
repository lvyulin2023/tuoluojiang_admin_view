...
export default {
  ...
  watch: {
    config: {
      handler(nVal) {
        if (nVal.data) {
          this.id = nVal.data.id
          this.rules.cid = nVal.data.cid
          this.rules.eid = nVal.data.eid
          this.rules.num = nVal.data.num
          this.rules.date = nVal.data.date
          this.rules.type_id = nVal.data.type_id
          this.rules.mark = nVal.data.mark
          this.rules.bill_cate_id = nVal.data.bill_cate_id
          if (nVal.data.attachs && nVal.data.attachs.length > 0) {
