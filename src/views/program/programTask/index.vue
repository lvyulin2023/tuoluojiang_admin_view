...
export default {
  ...
  methods: {
    ...
    disabledEndDate(time) {
      return time.getTime() < new Date(this.formData.start_date).getTime() - 8.64e7
    },

    setSort() {
      // ref一定跟table上面的ref一致
      const el = this.$refs.tableData.$el.querySelector('.el-table__fixed-body-wrapper tbody')
      const el2 = this.$refs.tableData.$el.querySelector('.el-table__body-wrapper tbody')
      //设置拖拽的参数
      const ops = {
        animation: 200, //动画时长
        ...
        onEnd: (evt) => {
          // 把树形的结构转为列表再进行拖拽
          const rowDown = this.activeRows[evt.oldIndex] // 移动的那个元素
          const rowUp = this.activeRows[evt.newIndex] // 移动的那个元素
          ...
          } else if (rowDown.pid == rowUp.pid) {
            // 拖动子级
            this.tableData.map((item) => {
              if (item.id == rowDown.pid) {
                item.children.map((it) => {
                  this.activeIds.push(it.id)
                })
              }
            })
          }
          this.moveElementAfter(this.activeIds, rowDown.id, rowUp.id)
        }
      }
      this.sortableFix = Sortable.create(el, ops)
      this.sortable = Sortable.create(el2, ops)
    },
     ...
     * @param row 当前的行数据
     * @param event 鼠标事件对象
     * @returns 如果拖动的行ID相同，返回false；如果跨级拖动，返回警告提示；否则返回undefined
     */
    handleMouseUp(row, event) {
      this.showDiv = false
      this.dragging = false
      document.removeEventListener('mousemove', this.handleMouseMove)
      this.rowUp = row
      if (this.rowDown.id == this.rowUp.id) {
        return false
      }
      if (this.rowDown.pid !== this.rowUp.pid) {
        return this.$message.warning('不允许跨级拖动!')
      }
      // 拖动一级
      if (this.rowDown.pid === 0) {
        this.tableData.map((item) => {
          this.activeIds.push(item.id)
        })
      } else if (this.rowDown.pid == this.rowUp.pid) {
        // 拖动子级
        this.tableData.map((item) => {
          if (item.id == this.rowDown.pid) {
            item.children.map((it) => {
              this.activeIds.push(it.id)
            })
          }
      ...
      this.moveElementAfter(this.activeIds, this.rowDown.id, this.rowUp.id)
    },
    /**
     * 鼠标离开事件处理函数
     *
