...
import { getcrudCateListApi, databaseSaveApi, databaseListApi, databaseDelApi, databaseCopyApi } from '@/api/develop'
export default {
  ...
  methods: {
    ...
    async createEntity(row, type) {
      ...
      this.formConfig[2].options = data.data.list
      const oldList = this.applicationTabData
      const list = oldList.slice(1, this.applicationTabData.length)
      this.formConfig[3].options = list
