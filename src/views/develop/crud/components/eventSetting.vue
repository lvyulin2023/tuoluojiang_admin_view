<template>
  <div>
    <div class="flex-between">
      ...
      <el-button size="small" type="primary" icon="el-icon-plus" @click="addFinance">新建触发器</el-button>
    </div>
    <!-- 筛选 -->
...
export default {
  ...
  mounted() {
    ...
    this.getActionList()
    this.getList()
    this.formDataInit.crud_id = Number(this.infoData.id)
  },
  methods: {
    // 获取触发器列表
    async getList() {
    ...
    async getEntity() {
      ...
      this.formConfig[0].options = data.data.list
    },
    ...
    async getOptions() {
      ...
      this.formConfig[1].options = data.data
    },
    ...
    getEvent(val) {
      ...
      return this.formConfig[1].options.length > 0 ? this.formConfig[1].options[index].label : '--'
    },

    // 新建触发器
    submit(data) {
      dataEventSaveApi(data).then((res) => {
        this.getList()
