...
export const uploader = async (file, uploadType, fileSize = helper.uploadSize, option) => {
  optionItem = option || {}
  return new Promise((resolve, reject) => {
    ...
    // 默认
    if (parseInt(uploadType) === 0 && parseInt(helper.uploadTypes.indexOf(type)) === -1) {
      Tips.msgError(`只能上传${helper.uploadTypes.join(',')}格式的文件`)
      return reject(false)
    }
    // 只能上传图片格式
...
export const baseUpload = {
  ...
  uploadMp4ToLocal(evfile, res, videoIng, option) {
    ...
    const formData = new FormData()
    formData.append('file', fileObject)
    formData.append('way', optionItem.way)
    formData.append('relation_type', optionItem.relation_type)
    formData.append('relation_id', optionItem.relation_id)
    formData.append('eid', optionItem.eid)
    videoIng(true, 100)
    return localUpload(formData)
  }
}
